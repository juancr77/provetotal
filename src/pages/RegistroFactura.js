...
function RegistroFactura() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Lógica de validación completa (sin extraer)
    ...
    if (!currentUser) return <div><p>Necesitas autenticarte.</p></div>;

    return (
        <div className="registro-factura-container">
            <h2>Registrar Nueva Factura</h2>
            <form onSubmit={handleSubmit} className="registro-form">
                <div className="form-group"><label>ID del Proveedor:</label><select value={idProveedor} onChange={(e) => handleProviderSelection(e.target.value)} required><option value="">-- Seleccione --</option>{proveedores.map(p => (<option key={p.id} value={p.idProveedor}>{p.idProveedor}</option>))}</select></div>
                <div className="form-group"><label>Nombre del Proveedor:</label><select value={idProveedor} onChange={(e) => handleProviderSelection(e.target.value)} required><option value="">-- Seleccione --</option>{[...proveedores].sort((a, b) => a.nombreCompleto.localeCompare(b.nombreCompleto)).map(p => (<option key={p.id} value={p.idProveedor}>{p.nombreCompleto}</option>))}</select></div>
                ...
                {/* --- NUEVOS CAMPOS --- */}
                <div className="form-group"><label>Dependencia:</label><input type="text" value={dependencia} onChange={(e) => setDependencia(e.target.value)} required /></div>
                <div className="form-group"><label>Forma de Pago:</label><select value={formaDePago} onChange={(e) => setFormaDePago(e.target.value)} required><option value="">-- Seleccione --</option><option value="Transferencia">Transferencia</option><option value="Efectivo">Efectivo</option><option value="Cheque">Cheque</option></select></div>
                <div className="form-group"><label>Fecha de Pago (Opcional):</label><input type="date" value={fechaDePago} onChange={(e) => setFechaDePago(e.target.value)} /></div>
                ...
                <div className="form-group"><label>Estatus:</label><select value={estatus} onChange={(e) => setEstatus(e.target.value)} required><option value="">-- Seleccione --</option><option value="Pendiente">Pendiente</option><option value="Pagada">Pagada</option></select></div>
                ...
                <div className="monto-group full-width">
                    ...
                    <div className="monto-controls">
                        ...
                        <div className="monto-input-container"><span>$</span><input type="number" value={monto} onChange={(e) => setMonto(e.target.value)} required /></div>
                        <button type="button" onClick={() => ajustarMonto(100)}>+100</button>
                        <button type="button" onClick={() => ajustarMonto(1000)}>+1000</button>
                    </div>
                </div>
